...
from itemData import get_items
...
class Vis():
    def __init__(self, context_file='KB/modifiers.tsv', lib_dir='',
...
def snippet_markup(doc_name, anno_doc, display_types={'SPAN_POSITIVE_PNEUMONIA_EVIDENCE'}):
...
def view_pycontext_output(input, vis=Vis('https://raw.githubusercontent.com/chapmanbe/pyConTextNLP/master/KB/pneumonia_modifiers.yml'),output='x'):
    if input is None:
    ...
    if isinstance(input, dict):
        ...
        view_pycontext_single_output(input, vis, output)


def view_pycontext_outputs(processed_docs, vis=Vis('https://raw.githubusercontent.com/chapmanbe/pyConTextNLP/master/KB/pneumonia_modifiers.yml')):
    doc_names = []
    docs_text = {}
    annotation_counts = {}
...
def view_pycontext_single_output(processed_doc, vis=Vis('https://raw.githubusercontent.com/chapmanbe/pyConTextNLP/master/KB/pneumonia_modifiers.yml'), output='x'):
    html, doc_text, total_annotations = vis.gen_html_from_context_doc(processed_doc)
    vis.write_html(html, str(output)+'.html')
    html = '''
			      <iframe src = "tmp/{}.html" frameborder="0" width = "850" height = "{}">
			         Sorry your browser does not support inline frames.
			      </iframe>'''.format(str(output), estimate_page_height(doc_text, total_annotations))
    display(HTML(html))
